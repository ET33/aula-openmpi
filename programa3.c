...
void calc_diferenca(int **matriz, int linha_maior,int coluna_maior,int *linha_menor,int
	*coluna_menor,int *valor_maior,int *valor_menor){
	...
	//printf("CCCCCCCCC(%d,%d) - MEU RANK EH %d\n",linha_maior,coluna_maior,myrank);
...
for (i=0; i < tam; i++) {
	for (j=0; j < tam; j++) {
		...
		if(myrank != 0){
			...
			printf("#%d > Mensagem recebida: mensagem[0]=%d from rank = %d with matrix pos = (%d,%d)\n",myrank, mensagem[0],mensagem[5],mensagem[1],mensagem[2]);
			...
			if (diferenca < mensagem[0]){
				...
				printf("#%d > Maior: (%d,%d) = %d | Menor: (%d,%d) = %d\n",myrank,linha_maior,coluna_maior, mensagem[3],linha_menor,coluna_menor,mensagem[4]);
			}
